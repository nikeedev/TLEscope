        ...
        if (text) {
            ...
            if (cts_ptr) {
                ...
                while ((cts_ptr = strstr(cts_ptr, "{")) && cts_ptr < block_end) {
                    if (config->custom_tle_source_count >= MAX_CUSTOM_TLE_SOURCES) break;
                    ...
                    if (name_ptr && name_ptr < obj_end && url_ptr && url_ptr < obj_end) {
                        ...
                        if (colon_name && colon_name < obj_end) {
                            ...
                            if (quote_start && quote_start < obj_end) sscanf(quote_start + 1, "%63[^\"]", config->custom_tle_sources[config->custom_tle_source_count].name);
                        }
                        
                        char* colon_url = strchr(url_ptr, ':');
                        if (colon_url && colon_url < obj_end) {
                            char* quote_start = strchr(colon_url, '"');
                            if (quote_start && quote_start < obj_end) sscanf(quote_start + 1, "%255[^\"]", config->custom_tle_sources[config->custom_tle_source_count].url);
                        }
            ...
            if (m_ptr) {
                ...
                while ((m_ptr = strstr(m_ptr, "{")) && m_ptr < block_end) {
                    ...
                    if (name_ptr && name_ptr < obj_end && 
                        lat_ptr && lat_ptr < obj_end && 
                        lon_ptr && lon_ptr < obj_end) {
                        
                        char* colon_name = strchr(name_ptr, ':');
                        if (colon_name && colon_name < obj_end) {
                            char* quote_start = strchr(colon_name, '"');
                            if (quote_start && quote_start < obj_end) sscanf(quote_start + 1, "%63[^\"]", markers[marker_count].name);
                        }
                        
                        char* colon_lat = strchr(lat_ptr, ':');
                        if (colon_lat && colon_lat < obj_end) sscanf(colon_lat + 1, "%f", &markers[marker_count].lat);
                        
                        char* colon_lon = strchr(lon_ptr, ':');
                        if (colon_lon && colon_lon < obj_end) sscanf(colon_lon + 1, "%f", &markers[marker_count].lon);

                        markers[marker_count].alt = 0.0f;
                        if (alt_ptr && alt_ptr < obj_end) {
                            char* colon_alt = strchr(alt_ptr, ':');
                            if (colon_alt && colon_alt < obj_end) sscanf(colon_alt + 1, "%f", &markers[marker_count].alt);
                        }
    ...
    if (config->custom_tle_source_count > 0) {
        ...
        for (int i = 0; i < config->custom_tle_source_count; i++) {
            fprintf(file, "    {\"name\": \"%s\", \"url\": \"%s\"}%s\n", 
                config->custom_tle_sources[i].name, config->custom_tle_sources[i].url,
                (i == config->custom_tle_source_count - 1) ? "" : ",");
        }
        fprintf(file, "    ],\n");
    }
